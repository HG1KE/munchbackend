...
class CustomerController extends Controller
{
    ...
    {
        $user = $this->user
            ->with('referral_customer_details')
            ->withCount(['orders', 'wishlist'])
            ->where('id', $request->user()->id)
            ->first();

        $referral = $user?->referral_customer_details;

        if ($referral && $referral->is_used == 0) {
            $validityType = $referral->customer_discount_validity_type;
            $validityValue = $referral->customer_discount_validity;
            $createdAt = $referral->created_at;

            if ($validityValue > 0 && $createdAt) {
                ...
                if (now()->gt($validUntil)) {
                    ...
                    $referral->update(['is_used' => 1]);

                    // Refresh relationship to reflect change in response
                    $user->load('referral_customer_details');
                }
            }
        }
   ...
   public function updateReferralCheck(Request $request)
   {
       ReferralCustomer::where('user_id', $request->user()->id)
