...
class POSController extends Controller
{
    ...
    public function place_order(Request $request): RedirectResponse
    {
       ...
       // $order->delivery_charge = $delivery_charge;
        ...
        $total_price_for_discount_validation = $total_price ?? 0;
        ...
        if (isset($cart['extra_discount']) && $cart['extra_discount_type'] == 'amount') {
            if ($cart['extra_discount'] > $total_price_for_discount_validation) {
                Toastr::error(translate('discount_can_not_be_more_than '). $total_price_for_discount_validation);
                return back();
            }
        }
        ...
        $delivery_charge = Helpers::get_delivery_charge(branchId: session()->get('branch_id') ?? 1, distance:  $distance, selectedDeliveryArea: $areaId, orderAmount:  $total_price + $total_tax_amount + $total_addon_tax);
