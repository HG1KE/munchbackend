            ...
            <div class="mt-3">
                <div class="row">
                    ...
                    @endif
                     ...
                     @php($config=\App\CentralLogics\Helpers::get_business_settings('pesapal'))
                    @if(isset($config) && $config['status'])
                        <div class="col-md-6 mb-4" style="cursor: pointer">
                            <div class="card">
                                <div class="card-body pt-2" style="height: 70px">
                                    <form method="POST" action="{!! route('pesapal.pay',['order_amount'=>$order_amount,'customer_id'=>$customer['id'],'callback'=>$callback]) !!}">
                                        {{ csrf_field() }}

                                        <button class="btn btn-block click-if-alone" type="submit" id="pesapal-button">

                                            <img width="200"
                                                 src="{{asset('public/assets/admin/img/pesapal.png')}}"/>
                                        </button>
                                    </form>
                                </div>
                            </div>
...
<script>
    $(document).ready(function (){
        ...
        } else if (payment_method === 'pesapal') {
            $('#pesapal-button').click();
        }
    });
</script>
