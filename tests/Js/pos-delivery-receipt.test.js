...
test('online snapshot prefers the saved order payload over cart state', function () {
    ...
    assert(app.indexOf('openSuccessModal(snapshotPrintJob(body), { offline: false })') !== -1
        || app.indexOf('openSuccessModal(snapshotPrintJob(body),{ offline: false })') !== -1, 'online success must snapshot the place-order response');
});
