...
class CustomerAuthController extends Controller
{
    ...
    public function registration(Request $request): JsonResponse
    {
        ...
        $referralData = $this->businessSetting
            ...
            ->orWhere('key', 'like', 'ref_earning_exchange_rate')
            ->orWhere('key', 'like', 'customer_referred_%')
            ->get();

        $referralData = array_column($referralData->toArray(), 'value', 'key');

        if ($refer_user && ($referralData['ref_earning_status'] ?? 0) == 1) {

                $createReferralData = [
                'user_id' => $user->id,
                'refer_by' => $refer_user->id,
                'ref_by_earning_amount' => $referralData['ref_earning_exchange_rate'] ?? 0,
            ];

            if (($referralData['customer_referred_discount_status'] ?? 0) == 1){
                $createReferralData = array_merge($createReferralData, [
                    'customer_discount_amount' => $referralData['customer_referred_discount_amount'] ?? 0,
                    'customer_discount_amount_type' => $referralData['customer_referred_discount_type'] ?? 'amount',
                    'customer_discount_validity' => $referralData['customer_referred_validity_value'] ?? 0,
                    'customer_discount_validity_type' => $referralData['customer_referred_validity_type'] ?? 'day',
                ]);
            }

            $this->referralCustomer->create($createReferralData);
    ...
    {
        $message = Helpers::order_status_update_message('register_with_referral_code_message');
        ...
        if ($value && isset($customerFcmToken)) {
            $data = [
                'title' => translate('Register with referral code'),
                'description' => $value,
                'order_id' => '',
                'image' => '',
                'type' => 'referral',
