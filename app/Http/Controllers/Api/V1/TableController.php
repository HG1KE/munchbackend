                ...
                try {
                    ...
                    if (isset($config) && $config['status'] == 1) {
                        ...
                        if ($branch && !empty($branch->phone)) {
                            // Get customer name
                            $customer_name = '';
                            if ($order->is_guest == 0 && isset($order->user_id)) {
                                $customer = \App\User::find($order->user_id);
                                if ($customer) {
                                    $customer_name = $customer->f_name . ' ' . $customer->l_name;
                                }
                            } elseif ($order->is_guest == 1 && isset($order->guest_id)) {
                                $guest = \App\Model\Guest::find($order->guest_id);
                                if ($guest) {
                            ...
                            $total_amount = $order->order_amount + $order->delivery_charge;
