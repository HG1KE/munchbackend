...
class ProductController extends Controller
{
    ...
    public function latestProducts(Request $request): JsonResponse
    {
        ...
        $products = ProductLogic::get_latest_products($request['limit'], $request['offset'], $request['product_type'], $request['name'], $request['category_ids'], $request['sort_by'], $request['is_halal']);
    ...
    {
        $products = ProductLogic::get_popular_products(limit: $request['limit'], offset: $request['offset'], product_type: $request['product_type'], name: $request['name'], is_halal: $request['is_halal']);
        $products['products'] = Helpers::product_data_formatting($products['products'], true);
        return response()->json($products, 200);
    }
    ...
    public function searchedProducts(Request $request): JsonResponse
    {
        ...
        if (count($products['products']) == 0) {
            ...
            $paginator = $this->product->active()
                ...
                ->when(isset($request['is_halal']) && $request['is_halal'] == 1, function ($query){
