...
class ConfigController extends Controller
{
    ...
    {
        $restaurantLogo = Helpers::get_business_settings('logo');
        $restaurantFavicon = Helpers::get_business_settings('fav_icon');
        ...
        return response()->json([
            ...
            // Admin Business Settings → fav icon (`fav_icon`) — filename under restaurant storage (Flutter parity).
            'restaurant_favicon' => $restaurantFavicon,
            // Backward-compatible absolute URLs for web/storefronts.
            ...
            // Explicit branding payload; additive only, does not change legacy keys.
            'branding' => [
                'logo' => $restaurantLogo,
            ...
            'branches' => $this->getBranchesWithAvailability(),
    ...
    /**
     * Get branches with availability data
     * @return array
     */
    private function getBranchesWithAvailability(): array
    {
        $branches = $this->branch->with('branch_time_schedules')
            ->where('status', 1)
            ->get(['id', 'name', 'email', 'longitude', 'latitude', 'address', 'coverage', 'status', 'image', 'cover_image', 'preparation_time']);

        return $branches->map(function ($branch) {
            $branchArray = $branch->toArray();
            
            // Add real-time availability data
            $branchArray['is_currently_open'] = Helpers::isBranchCurrentlyOpen($branch->id);
            $branchArray['can_order_today'] = Helpers::canStillOrderFromBranchToday($branch->id);
            
            // Add today's schedules for reference
            $dayOfWeek = Carbon::now()->dayOfWeek;
            $branchArray['today_schedules'] = Helpers::getBranchAvailabilitySchedules($branch->id, $dayOfWeek);
